...
URL = 'https://jobs.gem.com/11x-ai/job-id'
...
def test_gem_scan_preserves_rich_description_and_all_sections(monkeypatch, json_ld):
    ...
    monkeypatch.setattr(job_scanner.httpx, 'post', post)
...
def test_gem_api_failure_preserves_metadata_fallback(monkeypatch, body):
    monkeypatch.setattr(job_scanner, '_fetch_html', lambda _: FakeResponse(
        text='<meta name="description" content="Existing description">', url=URL))
    monkeypatch.setattr(job_scanner.httpx, 'post', lambda *a, **k: FakeResponse(json_data=body))
    assert job_scanner.scan_job_url(URL).description == 'Existing description'


def test_gem_http_failure_returns_none(monkeypatch):
    monkeypatch.setattr(job_scanner.httpx, 'post', lambda *a, **k: FakeResponse(status_code=503))
    assert job_scanner._fetch_gem_description(URL) is None


@pytest.mark.parametrize('url', ['https://example.com/job', 'https://jobs.gem.com/11x-ai',
    'https://jobs.gem.com.evil.test/11x-ai/job-id'])
def test_other_pages_do_not_fetch_gem_api(monkeypatch, url):
    def unexpected(*a, **k):
        raise AssertionError('Unexpected API request')
    monkeypatch.setattr(job_scanner.httpx, 'post', unexpected)
    assert job_scanner._fetch_gem_description(url) is None
