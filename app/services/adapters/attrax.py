...
# customer's own domain, e.g. careers.abbvie.com) locale-prefixes every real
# page (/en/jobs, /en/job/{slug}-jid-{id}) but the bare domain root doesn't
# redirect to reveal which locale — it just serves content based on
# Accept-Language/geo IP with siteUrlPrefix left as "/". Verified live: the
# root page's own nav still links the jobs search with an absolute,
# locale-prefixed URL (https://{host}/en/jobs?...), so that's scraped out of
# whatever page _detect_embedded already fetched rather than guessed.
...
def _fetch_jobs(board_key: str) -> list[str]:
    ...
    while len(urls) < _ATTRAX_MAX_JOBS:
        response = get_with_retry(f"https://{host}/{locale}/jobs", params={"page": page}, timeout=TIMEOUT)
        response.raise_for_status()
